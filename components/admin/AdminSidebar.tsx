...
export function AdminSidebar({ session }: AdminSidebarProps) {
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-64 flex-col border-r border-gray-200 bg-white lg:flex">
        ...
        <div className="flex h-16 items-center gap-3 border-b border-gray-200 px-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="truncate text-xs text-gray-500">Trang quản trị</p>
          </div>
        </div>
        ...
        <div className="border-t border-gray-200 p-4">
          <div className="rounded-lg bg-gray-50 p-3">
            <p className="text-xs font-semibold text-gray-700">{session.name}</p>
            <p className="mt-0.5 text-xs text-gray-500">{session.email}</p>
            <div className="mt-2 flex items-center gap-1">
              <span
                className={`inline-block h-2 w-2 rounded-full ${
                  session.role === 'SUPER_ADMIN' ? 'bg-purple-500' : 'bg-green-500'
                }`}
              />
      ...
      {/* Mobile Menu Button (will be implemented in header) */}
    </>
  );
}
